...
sme <- function()
{
    source("C:/Users/Jun/Dropbox/0-Research/0-metadata/docs/09-17/wlb-long-expire.R")
}
...
# use this for df that has only one workload_id
fdist_free_space_hist_single_workload <- function(df)
{
    # pick monitors
    pickedMons = pick_by_stride(unique(df$monitor_time), stride=1)
    df = subset(df, monitor_time %in% pickedMons)
    ...
    p = ggplot(df, aes(
        ...
        #geom_histogram(aes(fill=monitor_time), position='dodge') +
        #geom_density()+
        facet_grid(Free_Space_Dist_ID~monitor_time)+
        scale_x_continuous(breaks=seq(12, 28, by=2),
                           labels=(2^seq(12, 28, by=2)/1024))+
        xlab("Fragment Size(KB)")+
        theme(axis.text.x=element_text(angle=45,hjust=1))+
...
{
    df$Free_Space_Dist_ID = paste("alpha:", df$alpha, ",", "beta:", df$beta, sep="")
    df$Free_Space_Dist_ID = factor(df$Free_Space_Dist_ID)
    p <- ggplot(df, aes(x=Physical_start, 
        ...
        facet_grid(monitor_time~Free_Space_Dist_ID)+
        theme(axis.text.x=element_text(angle=45,hjust=1))
    print(p)
}
...
# the df can have only one workload setting
plot_ext_length_hist_single_workload <- function(df)
{
    # look at histogram of length
    df = subset(dfglb, Level_index == Max_level, drop=F)
...
{
    monitors = levels(list.dir[['_extlist']]$monitor_time)
    dfglb <<- df_filter(list.dir[['_extlist']], 
            p_np=8, p_nd=8, p_nf=16, p_nw=4096, 
            p_ws=64, p_wst=c(64), p_so=0, p_alpha=c(5,2), p_beta=c(5,2),
            #p_monitor_time=c("year00001.season00000")
                             #"year00002.season00000")
            p_monitor_time=monitors
            ) 

    dfglb$Free_Space_Dist_ID = paste("alpha:", dfglb$alpha, ",", "beta:", dfglb$beta, sep="")
    dfglb$Free_Space_Dist_ID = factor(dfglb$Free_Space_Dist_ID)
    #print(head(dfglb))
    #return()

    #print(c("Max_level summary\n"))
    #print(summary(dfglb$Max_level))
    #return()
    ...
    # directories has small extents
    #dfglb = subset(dfglb, Length < 2)
    #print(unique(dfglb$filepath))
    #return()

    #plot_ext_length_hist_single_workload(dfglb)
    #return()
...
main <- function()
{
    #list.dir <<- files2df("C:/Users/Jun/Documents/Large-Exp-Datasets/wlb.long.expire.complete")
    #save(list.dir, file="list.wlb_long_expire_complete.Rdata")
    #load(file="list.h7.caseb.Rdata", globalenv())

    #dfglb <<- list.dir[['_extstatssum']]
    #fdist_meta_data_blocks(dfglb)
    #return()

    #fdist_free_space_hist_single_workload(list.dir[['_freeblocks']])
